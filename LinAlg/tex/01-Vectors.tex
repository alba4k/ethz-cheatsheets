...
\includegraphics[width=0.32\textwidth]{../images/linear-combinations.png}
...
\paragraph{D 1.16} Let $d \in \mathbb{R}^m, d\ne 0$. The set $H_d=\{v\in \mathbb{R}:v \cdot d = 0 \}$ is called a \key{hyperplane through the origin}.

\paragraph{L 1.17} (\key{Triangle ineq.}) Let $v,w\in \mathbb{R}^m$. Then $\lVert v + w \rVert\le \lVert v \rVert + \lVert w \rVert$.
...
\paragraph{L 1.28} Let $v_1, \dots, v_m \in \mathbb{R}^m$ be linearly independent.
