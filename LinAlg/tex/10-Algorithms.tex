...
\paragraph{Direct Solution} $A$ is an $\mxn$ matrix in
...
\begin{itemize}
    ...
    \item All entries $r_{ij}$ "below the staircase" are $0$. This happens if:
        \begin{itemize}
            ...
            \item $i\le r$ and $j < j_i$ (entry is in the part of row $i$ to the left of column $j_i$).
        \end{itemize}
\end{itemize}
...
\paragraph{T 3.21} Let $A\in\mathbb{R}^{\mxn}$. $(R,j_1,\dots,j_r, M)$ is the output of Gauss-Jordan elimination. There exists an invertible $m\times m$ matrix $M$ such that $R=MA$ is in $\RREF(j_1,\dots,j_r)$.
...
\begin{equation}
    ...
    \left\lVert
    b-A\begin{pmatrix} \alpha_0 \\ \alpha_1 \end{pmatrix}
    \right\rVert
    ^2
    \label{eq:linregvec}
\end{equation}
...
If we need to find a polynomial $p(t) = \alpha_0 + \alpha_1 t + \dots + \alpha_k t^k$ which is as
close as possible to certain datapoints $(t_1, y_1), \dots, (t_m, y_m)$.
...
Our solution can be found by solving the normal equations:
$$A^\top A x = A^\top b$$
...
Let $A^{\mxn}$ be a matrix representing a linear transformation $L:\mathbb{R}^n\to\mathbb{R}^m$ given by $x\in\mathbb{R}^n\mapsto Ax\in\mathbb{R}m$, with both input and output written in the canonical bases as $x=\sum_{j=1}^{n}x_je_j$ and $Ax=\sum_{i=1}^{m}(Ax)_ie_i$.
...
For that, let $U\in\mathbb{R}^{\nxn}$ be the matrix with columns $u_1,\dots,u_n$ and let $V\in\mathbb{R}^{m\times m}$ be the matrix with columns $v_1,\dots,v_m$. Then $x=U\alpha$ and $L(x)=V\beta$, and so $\beta=V^{-1}AU\alpha$. Finally, this means that the matrix $B$, corresponding to the linear transformation $L$ written in the new bases, is $B=V^{-1}AU$.
