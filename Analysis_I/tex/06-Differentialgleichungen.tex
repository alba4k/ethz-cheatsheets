...
\subsection{Lösen einer linearen, homogengen DG}

Gegeben $a_n u^{(n)}(x) + \ldots + a_0 u(x) = 0$. Der \key{Ansatz} $u(x) = e^{\lambda x}$
(Eulerscher Ansatz) führt uns zur \key{charakteristische Gleichung}
...
\begin{itemize}
    ...
    $u(x) = \sum_{i=0}^{r} \left(\sum_{p=0}^{m_i-1} C_{ip}x^pe^{\lambda_i x}\right)$
    \item Falls $r$ reelle Nullstelle und $s$ \textit{Paare} komplexer Nullstellen ($a_j \pm ib_j$)
    mit Multiplizität $m_i$:
    $u(x) = \sum_{i=0}^{r}\left(\sum_{p=0}^{m_i - 1}C_{ip} x^p e^{\lambda_i x}\right) + \sum_{j=0}^{s} \left(\sum_{q=0}^{m_j-1}(A_{jq} x^q e^{a_j x} \sin(b_j x) + B_{jq} x^q e^{a_j x} \cos(b_j x))\right)$
\end{itemize}
...
$$F = m \cdot y'' = m \cdot a$$
wobei $y$ die Position des betrachteten Körpers und $a$ dessen Beschleiunigung sind.
...
Lösung der dazugehörigen homogenen DG die aufstretende Konstante durch
\textit{eine Funktion in der unab. Var. ersetze n}
