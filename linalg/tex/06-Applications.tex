...
\paragraph{L 6.2.1} Let $A\in\mathbb{R}^{m\times n}$. Let $x,y \in\colsp(A^\top)$.
...
\paragraph{D 6.3.3} A square matrix $Q\in\mathbb{R}^{n\times n}$ is an
...
product of vectors. In other words, if $Q\in\mathbb{R}^{n\times n}$ is
orthogonal, then, for all $x,y\in\mathbb{R}^n$, we have: $\norm{Qx}=\norm{x}$ and
$(Qx)^\top(Qy)=x^\top y$.

\paragraph{D 6.3.10} Let $A\in\mathbb{R}^{m\times n}$ with linearly
independent columns. The \key{QR decomposition} is given by $A=QR$, where $Q$
is an $m\times n$ matrix with orthonormal columns (they are the output of the
...
\paragraph{D 6.4.1} For $A\in\mathbb{R}^{m\times n}$ with $\rank(A)=n$, we define the
\key{pseudoinverse (full column rank)} $A^\dag\in\mathbb{R}^{n\times m}$ of $A$ as
$A^\dag=(A^\top A)^{-1}A^\top$ ($A^\dag A = I$).

\paragraph{D 6.4.3} For $A\in\mathbb{R}^{m\times n}$ with $\rank(A)=m$, we define
