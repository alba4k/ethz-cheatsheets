...
\paragraph{D 7.2.3} Let $A\in\mathbb{R}^{n\times n}$ be a square
...
$P\in\mathbb{R}^{n\times n}$ corresponding to a permutation $\sigma$, then
$\det(P)=\sgn(\sigma)$. We sometimes also write $\sgn(P)$.

    \item Given a triangular (either upper or lower) matrix
$T\in\mathbb{R}^{n\times n}$, we have $\det(T)=\prod^n_{k=1} T_{kk}$.

    \item If $Q\in\mathbb{R}^{n\times n}$ is an orthogonal
matrix, then $\det(Q)=1$ or $\det(Q)=-1$.
\end{enumerate}

\paragraph{T 7.2.5} Given a matrix $A\in\mathbb{R}^{n\times n}$, we have
$\det(A^\top)=\det(A)$.

\paragraph{T 7.2.6}
\begin{enumerate}
    \item $A\in\mathbb{R}^{n\times n}$ is invertible iff $\det(A)\not=0$.

    \item Given $A,B\in\mathbb{R}^{n\times n}$, we have $\det(AB)=\det(A)\det(B)$.

    \item Given $A\in\mathbb{R}^{n\times n}$ such that $\det(A)\not=0$, then $A$ is invertible, and $\det(A^{-1})=\frac{1}{\det(A)}$.
\end{enumerate}

\paragraph{D 7.3.1} Given $A\in\mathbb{R}^{n\times n}$, for each
...
\paragraph{P 7.3.2} Let $A\in\mathbb{R}^{n\times n}$, then for any
$1\leq i\leq n$: $\det(A)=\sum_{j=1}^{n}A_{ij}C_{ij}$.

\paragraph{P 7.3.3} Let $A\in\mathbb{R}^{n\times n}$ with
$\det(A)\not=0$ and let $C$ be the $n\times n$ matrix with the co-factors of
$A$ as entries. Then $A^{-1} = \frac{1}{\det(A)}C^\top$.

\paragraph{P 7.3.5} (\key{Cramer's rule}) Let
$A\in\mathbb{R}^{n\times n}$ such that $\det(A)\not=0$ and
